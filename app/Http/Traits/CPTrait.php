...
trait CPTrait
{
    ...
    {
        $req = [
          'amount'      => $amount, 'currency1' => $currency1, 'currency2' => $currency2, 'address' => $address, 'ipn_url' => $ipn_url,
    ...
    {
        $req = [
          'currency' => $currency, 'ipn_url' => $ipn_url,
        ];
    ...
    {
        $req = [
          'amount' => $amount, 'currency' => $currency, 'merchant' => $merchant, 'auto_confirm' => $auto_confirm ? 1 : 0,
        ];
    ...
    {
        $req = [
          'amount' => $amount, 'currency' => $currency, 'pbntag' => $pbntag, 'auto_confirm' => $auto_confirm ? 1 : 0,
        ];
        return $this->api_call('create_transfer', $req);
    }
    ...
    public function paywithcp($amount, $coin, $ui, $msg)
    {
        ...
        if ($msg == 'new') {
            ...
            $req = [
              'amount'    => $amount, 'currency1' => 'USD', 'currency2' => $coin, 'buyer_email' => $user->email, 'buyer_name' => $user->name,
              'address'   => $address, // leave blank send to follow your settings on the Coin Settings page
              'item_name' => 'Account deposit', 'ipn_url' => $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'_p',
            ];
            ...
            if ($result['error'] == 'ok') {
                ...
                return view('user.payment')->with([
                  'title'     => 'Complete Payment', 'amount' => $result['result']['amount'], 'coin' => $coin,
                  'p_address' => $result['result']['address'], 'p_qrcode' => $result['result']['qrcode_url'],
                  'settings'  => Settings::where('id', '1')->first(),
                ]);
              ...
              print 'Error: '.$result['error']."\n";
                ...
                if ($result['error'] == 'ok') {
                    ...
                    if ($status >= 100 || $status == 2) {
                        ...
                        //Update txn info on cp_txns
                        Cp_transaction::where('id', $txn->id)->update([
                          'status' => $result['result']['status'], 'status_text' => $result['result']['status_text'],
                        ]);
                        //get user
                        ...
                        if ( ! empty($user->ref_by) || $user->ref_by != "0") {
                            ...
                            User::where('id', $duser->id)->update([
                              'account_bal' => $duser->account_bal + $earnings,
                            ]);
                            //create history
                            Tp_Transaction::create([
                              'user' => $duser->id, 'plan' => "Credit", 'amount' => $earnings, 'type' => "Ref_bonus",
                            ]);
    ...
    public function cpwithdraw($amount, $coin, $wallet, $ui, $to_withdraw)
    {
        ...
        if ($result['error'] == 'ok') {
            ...
            $txn->currency2 = $coin;
            $txn->save();
            return redirect()->back()->with('message', 'Withrawal created successful! You will recieve your funds soon.');
        } else {
            //print 'Error: '.$result['error']."\n";
            return ($result['error']);
        }
    ...
    {
        $req = [
          'amount'       => $amount, 'add_tx_fee' => $add_tx_fee, 'currency' => $currency, 'currency2' => $currency2, 'address' => $address,
          'auto_confirm' => $auto_confirm ? 1 : 0, 'ipn_url' => $ipn_url,
        ];
            ...
            if ($this->getDownlinePlans($dwln->id, $amount) == true) {
                ...
                if ($upln->amount >= $cm_pc) {
                    ...
                    User::where('id', $sponsor)->update([
                      'account_bal' => $spnsr->account_bal + $matching_bonus, 'ref_bonus' => $spnsr->ref_bonus + $matching_bonus,
                    ]);
                    //create history
                    Tp_Transaction::create([
                      'user' => $spnsr->id, 'plan' => "Credit", 'amount' => $matching_bonus, 'type' => "Matching bonus",
                    ]);
                    //mark the downline bonus_matched field = matched
                    User::where('id', $dwln->id)->update([
    ...
    public function creditLeadershipBonus($sponsor, $amnt)
    {
        ...
        foreach ($array as $entry) {
            ...
            //create history
            Tp_Transaction::create([
              'user' => $entry->id, 'plan' => "Credit", 'amount' => $earnings, 'type' => "Leadership bonus",
            ]);
            $level .= $level + 1;
            if ($level == 11) {
                break;
