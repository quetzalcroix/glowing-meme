  ...
  <div class="main-panel">
    <div class="content bg-{{auth('admin')->user()->dashboard_style}}">
      <div class="page-inner">
        <div class="mt-2 mb-4">
          <h1 class="title1 text-{{$text}}">TRO Plans</h1>
        </div>
              ...
              <div class="pricing-table purple border bg-{{auth('admin')->user()->dashboard_style}} shadow-lg">
                ...
                <div class="pricing-features">
                  <div class="feature text-{{$text}}">
                    Minimum Deposit:<span class="text-{{$text}}">{{$plan->min_price}} {{$settings->currency}}</span>
                  </div>
                  <div class="feature text-{{$text}}">Maximum Deposit:<span
                      class="text-{{$text}}">{{$plan->max_price}} {{$settings->currency}}</span></div>
                  <div class="feature text-{{$text}}">Minimum Return:<span class="text-{{$text}}">{{$plan->minr}} %</span></div>
                  <div class="feature text-{{$text}}">Maximum Return:<span class="text-{{$text}}">{{$plan->maxr}} %</span></div>
                  <div class="feature text-{{$text}}">Gift Bonus:<span class="text-{{$text}}">{{$plan->gift}} {{$settings->currency}}</span></div>
                ...
                <div class="text-center">
                  <a href="#" data-toggle="modal" data-target="#editplansModal{{$plan->id}}" class="btn btn-primary"><i
                      class="text-white flaticon-pencil"></i></a> &nbsp;
                  <a href="{{ url('rangda/v2/dashboard/trashplan') }}/{{$plan->id}}" class="btn btn-danger"><i class="text-white fa fa-times"></i></a>
                </div>
                  ...
                  <div class="modal-content bg-{{auth('admin')->user()->dashboard_style}}">
                    ...
                    <div class="modal-body">
                      <form role="form" method="post" action="{{ route('updateplan') }}">
                        ...
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->name}}" type="text" name="name" required><br/>
                        <h5 class="text-{{$text}}">Plan price</h5>
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->price}}" type="text" name="price" required><br/>
                        <h5 class="text-{{$text}}">Plan Minimum Price</h5>
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->min_price}}" type="text" name="min_price" required><br/>
                        <h5 class="text-{{$text}}">Plan Maximum Price</h5>
                        <input style="padding:5px;" class="form-control  text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->max_price}}" type="text" name="max_price" required><br/>
                        <h5 class="text-{{$text}}"> Minimum Return</h5>
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->minr}}" placeholder="Enter minimum return" type="text" name="minr" required><br/>
                        <h5 class="text-{{$text}}"> Maximum Return</h5>
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->maxr}}" placeholder="Enter maximum return" type="text" name="maxr" required><br/>
                        <h5 class="text-{{$text}}"> Gift Bonus</h5>
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->gift}}" placeholder="Enter Additional Gift Bonus" type="text" name="gift" required><br/>
                        ...
                        <input style="padding:5px;" class="form-control text-{{$text}} bg-{{auth('admin')->user()->dashboard_style}}"
                               value="{{$plan->increment_amount}}" placeholder="top up amount" type="text" name="t_amount" required><br/>
