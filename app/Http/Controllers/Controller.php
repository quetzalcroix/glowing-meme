...
class Controller extends BaseController
{
    ...
    public function dashboard(Request $request)
    {
            ...
            if ($ref_by != Auth::user()->id) {
                ...
                if (count($ag) > 0) {
                    ...
                    $agent_id = DB::table('agents')->insertGetId([
                      'agent' => $ref_by, 'created_at' => Carbon::now(), 'updated_at' => Carbon::now(),
                    ]);
        ...
        foreach ($usf as $usf) {
            ...
            if ($usf->ref_link == '') {
                User::where('id', $usf->id)->update([
                  'ref_link' => $settings->site_address.'/register/'.uniqid("digest-promo-"),
                ]);
            }
            //give reg bonus if new
//            if ($usf->created_at->diffInDays() < 2 && $usf->signup_bonus != "received") {
            /*
            if ($usf->signup_bonus != "received") {
                User::where('id', $usf->id)->update([
                  'bonus'        => $usf->bonus + $settings->signup_bonus, 'account_bal' => $usf->account_bal + $settings->signup_bonus,
        ...
        //sum total deposited
        $total_deposited = DB::table('deposits')->select(DB::raw("SUM(amount) as count"))->where('user', Auth::user()->id)->where('status',
          'Processed')->get();
        ...
        return view('user.dashboard')->with([
            //'earnings'=>$earnings,
            'title'     => 'User panel', 'ref_earnings' => $ref_earnings, 'deposited' => $total_deposited, 'total_bonus' => $total_bonus,
            'user_plan' => $user_plan, 'user_plan_active' => $user_plan_active, 'upplan' => Plans::where('id', Auth::user()->promo_plan)->first(),
            'uplan'     => Plans::where('id', Auth::user()->plan)->first(), 'settings' => Settings::where('id', '=', '1')->first(),
        ]);
        //}
    }
    ...
    {
        return view('user.subtrade')->with([
          'title'    => 'Subscription Trade', 'subscriptions' => Mt4Details::where('client_id', auth::user()->id)->orderBy('id', 'desc')->get(),
          'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    public function delsubtrade($id)
    {
        ...
        return redirect()->back()->with('message', 'MT4 Details Sucessfully Deleted');
    }
    ...
    public function savemt4details(Request $request)
    {
        ...
        $mt4->save();
        return redirect()->back()->with('message', 'MT4 Details Submitted Successfully, Please wait for the system to validate your credentials');
    }

    //Return withdrawals route
    public function deposits()
    {
        return view('user.deposits')->with([
          'title'    => 'Deposits', 'deposits' => Deposit::where(['user' => Auth::user()->id])->orderBy('id', 'desc')->get(),
    ...
    {
        return view('user.withdrawals')->with([
          'title'    => 'withdrawals', 'withdrawals' => Withdrawal::where('user', Auth::user()->id)->orderBy('id', 'desc')->get(),
          'settings' => Settings::where('id', '=', '1')->first(),
          'wmethods' => Wdmethod::where('type', 'withdrawal')->where('status', 'enabled')->get(),
        ]);
    }
    ...
    {
        return view('user.mplans')->with([
          'title' => 'Main Plans', 'plans' => Plans::where('type', 'main')->get(), 'settings' => Settings::where('id', '1')->first(),
        ]);
    }
    ...
    public function myplans()
    {
        ...
        return view('user.myplans')->with([
          'title' => 'Your packages', 'plans' => User_plans::where('user', Auth::user()->id)->get(),
          'cplan' => User_plans::where('id', Auth::user()->user_plan)->first(), 'settings' => Settings::where('id', '1')->first(),
        ]);
        //fect user
        /*
    ...
    {
        return view('pplans')->with([
          'title' => 'Promo Plans', 'plans' => Plans::where('type', 'promo')->get(), 'settings' => Settings::where('id', '1')->first(),
        ]);
    }
    ...
    public function joinplan(Request $request)
    {
        ...
        if ($user->account_bal < $plan_price) {
            ...
            return redirect()->route('deposits')->with('message', 'Your account is insufficient to purchase this plan. Please make a deposit.');
        }
        if ($plan->type == 'Main') {
            //debit user
            User::where('id', $user->id)->update([
            ...
            //create history
            Tp_Transaction::create([
              'user' => $user->id, 'plan' => $plan->name, 'amount' => $plan_price, 'type' => "Plan purchase",
            ]);
            //save user plan
            $userplanid = DB::table('user_plans')->insertGetId([
              'plan'         => $plan->id, 'user' => Auth::user()->id, 'amount' => $plan_price, 'active' => 'yes',
              'inv_duration' => $request['duration'], 'activated_at' => Carbon::now(), 'last_growth' => Carbon::now(), 'created_at' => Carbon::now(),
              'updated_at'   => Carbon::now(),
            ]);
            User::where('id', Auth::user()->id)->update([
              'plan' => $plan->id, 'user_plan' => $userplanid, 'entered_at' => Carbon::now(),
            ]);
        ...
        return redirect()->back()->with('message', 'You successfully purchased a plan and your plan is now active.');
    }

    //return add account form
    public function support()
    {
        return view('user.support')->with([
          'title' => 'Support', 'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    {
        return view('user.updateacct')->with([
          'title' => 'Update account details', 'settings' => Settings::where('id', '=', '1')->first()
        ]);
    }
    ...
    public function updateacct(Request $request)
    {
        User::where('id', $request['id'])->update([
          'bank_name'   => $request['bank_name'], 'account_name' => $request['account_name'], 'account_no' => $request['account_number'],
          'btc_address' => $request['btc_address'], 'eth_address' => $request['eth_address'],
        ]);
        return redirect()->back()->with('message', 'User updated Sucessful');
    }

    public function changepassword(Request $request)
    {
        return view('includes.changepassword')->with(['title' => 'Change Password', 'settings' => Settings::where('id', '=', '1')->first()]);
    }
    ...
    public function updatepass(Request $request)
    {
        ...
        $this->validate($request, [
          'password_confirmation' => 'same:password', 'password' => 'min:6',
        ]);
        User::where('id', $request['id'])->update([
    ...
    {
        return view('includes.referuser')->with([
          'title' => 'Refer user', 'settings' => Settings::where('id', '=', '1')->first()
        ]);
    }
                    ...
                    if ($settings->weekend_trade == 'true') {
                        if ($plan->last_growth <= $togrow) {
                            ...
                            User::where('id', $plan->user)->update([
                              'roi' => $user->roi + $increment, 'account_bal' => $user->account_bal + $increment,
                            ]);
