...
class SomeController extends Controller
{
    ...
    {
        return view('user.thistory')->with([
          't_history' => Tp_Transaction::where('user', Auth::user()->id)->where('type', 'ROI')->orderBy('id', 'desc')->get(),
          'title'     => 'Trading History', 'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    public function accounthistory()
    {
        return view('user.transactions')->with([
          't_history'   => Tp_Transaction::where('user', Auth::user()->id)->where('type', '<>', 'ROI')->orderBy('id', 'desc')->get(),
          'withdrawals' => Withdrawal::where('user', Auth::user()->id)->orderBy('id', 'desc')->get(),
          'deposits'    => Deposit::where('user', Auth::user()->id)->orderBy('id', 'desc')->get(), 'title' => 'Account Transactions History',
    ...
    public function notification()
    {
        return view('user.notification')->with([
          'Notif'    => Notification::where('user_id', Auth::user()->id)->orderBy('id', 'desc')->paginate(12), 'title' => 'Notification',
          'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    public function profile()
    {
        ...
        return view('user.profile')->with([
          'userinfo' => $userinfo, 'title' => 'Profile', 'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    public function updateprofile(Request $request)
    {
        User::where('id', Auth::user()->id)->update([
          'name' => $request->name, 'dob' => $request->dob, 'phone' => $request->phone, 'address' => $request->address,
        ]);
        return redirect()->back()->with('message', 'Profile Information Updated Sucessfully!');
    }
    ...
    public function delnotif($id)
    {
        ...
        return redirect()->back()->with('message', 'Message Sucessfully Deleted');
    }

    //save CoinPayments credentials to DB
    public function updatecpd(Request $request)
    {
        Cp_transaction::where('id', '1')->update([
          'cp_p_key'      => $request['cp_p_key'], 'cp_pv_key' => $request['cp_pv_key'], 'cp_m_id' => $request['cp_m_id'],
          'cp_ipn_secret' => $request['cp_ipn_secret'], 'cp_debug_email' => $request['cp_debug_email'],
        ]);
    ...
    public function savevdocs(Request $request)
    {
        $this->validate($request, [
          'idcard' => 'mimes:jpg,jpeg,png|max:4000|image', 'passport' => 'mimes:jpg,jpeg,png|max:4000|image',
        ]);
        $settings = Settings::where('id', '=', '1')->first();
        ...
        if ($request->hasfile('idcard')) {
            ...
            $documentArray = explode(".", $filename1);
            $ext = array_pop($documentArray);
            ...
            if (in_array($ext, $whitelist)) {
                ...
                return redirect()->back()->with('message', 'Unaccepted ID Card Image Uploaded, try renaming the image file');
            }
        }
        if ($request->hasfile('passport')) {
            $document2 = $request->file('passport');
            $filename2 = $document2->getClientOriginalName();
            $passportArray = explode(".", $filename2);
            $ext = array_pop($passportArray);
            $whitelist = ['jpeg', 'jpg', 'png'];
            if (in_array($ext, $whitelist)) {
                if ($settings->location == "Email") {
                ...
                return redirect()->back()->with('message', 'Unaccepted Passport Image Uploaded, try renaming the image file');
            }
        }
        if ($settings->location == "Email") {
            $data = [
              'document' => $document1, 'document1' => $document2,
            ];
        ...
        //update user
        User::where('id', Auth::user()->id)->update([
          'id_card' => $cardname, 'passport' => $passname, 'account_verify' => 'Under review',
        ]);
        return redirect()->back()->with('message', 'Action Sucessful! Please wait for system to validate your submission.');
    }
    ...
    public function deposit(Request $request)
    {
        ...
        $paymentIntent = PaymentIntent::create([
          'amount'   => $amt, 'currency' => 'usd', 'payment_method_types' => ['card'],
          // Verify your integration in this guide by including this parameter
          'metadata' => ['integration_check' => 'accept_a_payment'],
        ]);
        $output = [
          'publishableKey' => $publickey, 'clientSecret' => $paymentIntent->client_secret,
        ];
        ...
        return redirect()->route('payment')->with([
          'title' => 'Make deposit', 'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    public function payment(Request $request)
    {
        return view('user.payment')->with([
          'amount'   => $request->session()->get('amount'), 'payment_mode' => $request->session()->get('payment_mode'),
          'pay_type' => $request->session()->get('pay_type'), 'plan_id' => $request->session()->get('plan_id'),
          'intent'   => $request->session()->get('intent'), 'title' => 'Make deposit', 'settings' => Settings::where('id', '=', '1')->first(),
        ]);
    }
    ...
    public function savedeposit(Request $request)
    {
        ...
        if ($request->hasfile('proof')) {
            ...
            $depositProof = $file->getClientOriginalName();
            $array = explode(".", $depositProof);
            $ext = array_pop($array);
            $whitelist = ['jpeg', 'jpg', 'png'];
            if (in_array($ext, $whitelist)) {
                if ($settings->location == "Email") {
                    $proofname = $strtxt.$depositProof.time();
                    $data = [
                      'document' => $file
                    ];
        ...
        return redirect()->route('deposits')->with('message', 'Action Sucessful! Please wait for system to validate this transaction.');
    }
    ...
    public function withdrawal(Request $request)
    {
        ...
        if ($request['amount'] < $method->minimum) {
            return redirect()->back()->with("message", "Sorry, The minimum amount is $settings->currency$method->minimum.");
        }
        //get user last investment package
        $last_user_plan = User_plans::where('user', Auth::user()->id)->where('active', 'yes')->orderby('activated_at', 'ASC')->first();
        //get user
        $user = User::where('id', Auth::user()->id)->first();
        $amount = $request['amount'];
        $ui = $user->id;
        if (empty($user->btc_address) && empty($user->ltc_address) && empty($user->eth_address) && empty($user->account_number)) {
            return redirect()->route('acountdetails')->with('message', 'You must set up your coins wallet address before you can withdraw.');
        }
        ...
        $dp->user = $user->id;
        $dp->save();
        return redirect()->back()->with('message', 'Action Sucessful! Please wait for system to process your request.');
    }
}
