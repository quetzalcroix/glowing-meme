    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{Auth('admin')->User()->dashboard_style}}">
        <form style="padding:3px;" role="form" method="post" action="{{route('sendmailtoall')}}">
          ...
          <textarea class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" name="message" row="3"
                    placeholder="Type Message here" required></textarea><br/>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{Auth('admin')->User()->dashboard_style}}">
        <form style="padding:3px;" role="form" method="post" action="{{action('App\Http\Controllers\Admin\SettingsController@addwdmethod')}}">
          <input style="padding:5px;" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}"
                 placeholder="Enter method name" type="text" name="name" required><br/>
          <input style="padding:5px;" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" placeholder="Minimum amount $"
                 type="text" name="minimum" required><br/>
          <input style="padding:5px;" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" placeholder="Maximum amount $"
          ...
          <input style="padding:5px;" class="form-control bg-{{Auth('admin')->User()->dashboard_style}} text-{{$text}}" placeholder="Payout duration"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{Auth('admin')->User()->dashboard_style}}">
        <form style="padding:3px;" role="form" method="post" action="{{ route('addplan') }}">
          ...
          <input style="padding:5px;" class="form-control text-{{$text}} bg-{{Auth('admin')->User()->dashboard_style}}" placeholder="Enter Plan name"
                 type="text" name="name" required><br/>
          <h5 class="text-{{$text}}">Plan price</h5>
          <input style="padding:5px;" class="form-control text-{{$text}} bg-{{Auth('admin')->User()->dashboard_style}}" placeholder="Enter Plan price"
          ...
          <input style="padding:5px;" placeholder="Enter Plan minimum price"
                 class="form-control text-{{$text}} bg-{{Auth('admin')->User()->dashboard_style}}" type="text" name="min_price" required><br/>
          ...
          <input style="padding:5px;" class="form-control text-{{$text}} bg-{{Auth('admin')->User()->dashboard_style}}"
                 placeholder="Enter Plan maximum price" type="text" name="max_price" required><br/>
          ...
          <input style="padding:5px;" class="form-control text-{{$text}} bg-{{Auth('admin')->User()->dashboard_style}}" placeholder="top up amount"
