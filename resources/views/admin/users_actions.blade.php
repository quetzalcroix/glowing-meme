  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header bg-{{auth('admin')->user()->dashboard_style}}">
        <h4 class="modal-title" style="text-align:center;">Credit/Debit User account</h4>
      ...
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}}">
        <form style="padding:3px;" role="form" method="post" action="{{route('topup')}}">
          <input style="padding:5px;" class="form-control bg-{{auth('admin')->user()->dashboard_style}} text-{{$text}}" value="{{$list->name}}"
                 type="text" disabled><br/>
          <input style="padding:5px;" class="form-control bg-{{auth('admin')->user()->dashboard_style}} text-{{$text}}" placeholder="Enter amount"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}}">
        ...
        <form style="padding:3px;" role="form" method="post" action="{{action('App\Http\Controllers\Admin\LogicController@sendmailtooneuser')}}">
          <input type="hidden" name="user_id" value="{{$list->id}}">
          <textarea placeholder="Type your message here" class="form-control bg-{{auth('admin')->user()->dashboard_style}} text-{{$text}}"
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}}">
        <form style="padding:3px;" role="form" method="post" action="{{route('edituser')}}">
          ...
          <input style="padding:5px;" class="form-control bg-{{auth('admin')->user()->dashboard_style}} text-{{$text}}" value="{{$list->email}}"
                 type="text" name="email" required><br/>
          <h5 class=" text-{{$text}}">Phone Number</h5>
          <input style="padding:5px;" class="form-control bg-{{auth('admin')->user()->dashboard_style}} text-{{$text}}" value="{{$list->phone}}"
                 type="text" name="phone" required><br/>
          <h5 class=" text-{{$text}}">Referral link</h5>
          <input style="padding:5px;" class="form-control bg-{{auth('admin')->user()->dashboard_style}} text-{{$text}}" value="{{$list->ref_link}}"
                 type="text" name="ref_link" required><br/>
          <input type="hidden" name="_token" value="{{ csrf_token() }}">
          <input type="hidden" name="user_id" value="{{$list->id}}">
...
<div id="resetpswdModal{{$list->id}}" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header bg-{{auth('admin')->user()->dashboard_style}}">
        <h4 class="modal-title text-{{$text}}">Reset Password</h4>
        <button type="button" class="close text-{{$text}}" data-dismiss="modal">&times;</button>
      </div>
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}}">
        <p class="text-{{$text}}">Are you sure you want to reset password for {{$list->name}} {{$list->l_name}} to <span
            class="text-primary font-weight-bolder">
            great-trade
          </span></p>
        <a class="btn btn-{{$text}}" href="{{ url('rangda/v2/dashboard/resetpswd') }}/{{$list->id}}">
          Reset Now
        </a>
      </div>
    </div>
...
<div id="switchuserModal{{$list->id}}" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}}">
        <a class="btn btn-{{$text}}" href="{{ url('rangda/v2/dashboard/switchuser') }}/{{$list->id}}">Proceed</a>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}}">
        ...
        <a class="btn btn-{{$text}}" href="{{ url('rangda/v2/dashboard/clearacct') }}/{{$list->id}}">Proceed</a>
      </div>
    </div>
  </div>
</div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body bg-{{auth('admin')->user()->dashboard_style}} p-3">
        ...
        <a class="btn btn-danger" href="{{ url('rangda/v2/dashboard/delsystemuser') }}/{{$list->id}}">Yes i'm sure</a>
      </div>
    </div>
  </div>
</div>
